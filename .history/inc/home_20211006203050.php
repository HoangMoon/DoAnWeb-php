								...
								<div class="col-md-4 product-men mt-5 product-item">
									<div class="men-pro-item simpleCart_shelfItem">
										...
										<div class="item-info-product text-center border-top mt-4">
											...
											<?php 
												$sql_soluongban = mysqli_query($con,"SELECT COUNT(tbl_donhang.soluong) AS 'daban'  FROM tbl_donhang" );
												$_count = mysqli_num_rows($sql_soluongban);
														$row_ban = mysqli_fetch_array($sql_soluongban);
														if($row_sanpham['sanpham_id'] = $sanpham_id) {
											?>
														<span class="number-buy" style="font-size: 14px">
														<?php
														$_SESSION['soluong'] = $row_ban['daban'];
											...
											<form action="?quanly=giohang" method="post">
												<fieldset>
													...
													<div class="btn-buy"><i class="fa fa-cart-plus" aria-hidden="true"></i>
														<input  type="submit" name="themgiohang" value="Mua ngay" class="button" /></div>
												</fieldset>
											</form>
											</div>
										</div>
